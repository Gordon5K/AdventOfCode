...
namespace AOC
{
    class Day4
    {
        //readonly string _input = @"aaaaa-bbb-z-y-x-123[abxyz]
        //a-b-c-d-e-f-g-h-987[abcde]
        //not-a-real-room-404[oarel]
        //totally-real-room-200[decoy]"

        readonly string _input = @"fubrjhqlf-edvnhw-dftxlvlwlrq-803[wjvzd]
...
zsxyfgqj-hqfxxnknji-idj-xytwflj-359[jxfin]";
        ...
        public object Task1()
        {
            List<int> sectorIds = new();
            foreach(string str in _input.Split(new char[] { '\n' }))
            {
                Dictionary<char, int> chars = new();
                List<char> sortedChars = null;

                string line = str.Replace("\r", string.Empty).Trim();

                int lastLetterIndex = line.LastIndexOf("-");
                string stringPart = line.Substring(0, lastLetterIndex);
                string codeAndCheckSum = line.Substring(lastLetterIndex + 1, line.Length - lastLetterIndex -1);
                int indexOfBracket = codeAndCheckSum.IndexOf('[');
                int sectorId = int.Parse(codeAndCheckSum.Substring(0, indexOfBracket));
                string checkSum = codeAndCheckSum.Substring(indexOfBracket + 1, codeAndCheckSum.Length - indexOfBracket - 2);

                foreach (string charPart in stringPart.Split(new char[] { '-' }, StringSplitOptions.RemoveEmptyEntries))
                {
                    foreach (char c in charPart)
                    {
                ...
                sortedChars = chars.OrderByDescending(s => s.Value).ThenBy(s => s.Key).Take(5).Select(s => s.Key).ToList();
                if (checkSum.Intersect(sortedChars).Count() == checkSum.Length)
                {
        ...
        public int Task2()
        {
            Dictionary<string, int> decodedRooms = new();
            foreach (string str in _input.Split(new char[] { '\n' }))
            {
                Dictionary<char, int> chars = new();
                List<char> sortedChars = null;
                ...
                if (checkSum.Intersect(sortedChars).Count() == checkSum.Length)
                {
                    ...
                    if (!decodedRooms.ContainsKey(decoded.ToString()))
                    {
                        decodedRooms.Add(decoded.ToString(), sectorId);
                    }
                }

            }

            var theRoom = decodedRooms.FirstOrDefault(s => s.Key.Contains("northpole") && s.Key.Contains("object") && s.Key.Contains("storage"));

            return theRoom.Value;
        }
