...
namespace AOC
{
    class Day10
    {
        readonly string _input = @"bot 135 gives low to bot 27 and high to bot 166
...
bot 111 gives low to bot 125 and high to bot 51";
        ...
        readonly Regex _passChip = new(@"value (?<value>\d*) goes to bot (?<bot>\d*)");
        readonly Regex _passDoubleChip = new(@"bot (?<bot>\d*) gives low to (?<ldest>output|bot) (?<ldestid>\d*) and high to (?<hdest>output|bot) (?<hdestid>\d*)");

        readonly List<Bot> _bots = new();
        List<Output> _outputs = new();
        ...
        public object Task1()
        {
            ...
            for (int l = 0; l < lines.Length; l++)
            {
                ...
                var match = _passChip.Match(line);
                if (match.Success)
                {
                    int botId = int.Parse(match.Groups["bot"].Value);
                    Bot bot = (Bot)GetOrCreateBot(true, botId);
                ...
                {
                    match = _passDoubleChip.Match(line);

                    int botId = int.Parse(match.Groups["bot"].Value);
                    Bot bot = (Bot)GetOrCreateBot(true, botId);

                    string lDest = match.Groups["ldest"].Value;
                    int lDestId = int.Parse(match.Groups["ldestid"].Value);
                    string hDest = match.Groups["hdest"].Value;
                    int hDestId = int.Parse(match.Groups["hdestid"].Value);

                    bot.LDest = lDestId;
                    bot.LDestIsOutput = lDest != "bot";
                    bot.HDest = hDestId;
                    bot.HDestIsOutput = hDest != "bot";
                }
            }

            bool allHaveOne = false;
            while (!allHaveOne)
            {                
                foreach(Bot bot in _bots)
                {
                    ...
                    {
                        allHaveOne = false;
                        Part lPart = GetOrCreateBot(!bot.LDestIsOutput, bot.LDest);
                        lPart.Values.Add(bot.Low);

                        Part hPart = GetOrCreateBot(!bot.HDestIsOutput, bot.HDest);
                        hPart.Values.Add(bot.High);
        ...
        private Part GetOrCreateBot(bool isBot, int id)
        {
            if (isBot)
            {
                Bot bot = _bots.FirstOrDefault(s => s.Id == id);
            ...
            {
                Output output = _outputs.FirstOrDefault(s => s.Id == id);

                if (output == null)
                {
                    output = new Output { Id = id };
                    _outputs.Add(output);
                }
                return output;
            }
        ...
        public object Task2()
        {
            _outputs = _outputs.OrderBy(s=>s.Id).Take(3).ToList();
        ...
        private class Part
        {
            ...
            public int LDest { get; set; }
            public int HDest { get; set; }

            public bool LDestIsOutput { get; set; }

            public bool HDestIsOutput { get; set; }

            public bool HasTwo => Values.Count == 2;
        }

        private sealed class Bot : Part { }

        private sealed class Output : Part { }
