...
namespace AOC._2018
{
    class Day10
    {
        readonly string _input = @"position=< 50200,  10144> velocity=<-5, -1>
...
position=<-19880,  30162> velocity=< 2, -3>";
        ...
        private List<Light> GetValues()
        {
            ...
            {
                Regex r = new(@"position=<\s*(-?\d+),\s*(-?\d+)> velocity=<\s*(-?\d+),\s*(-?\d+)>");
                Match m = r.Match(line);
                var light = new Light(int.Parse(m.Groups[1].ToString()), int.Parse(m.Groups[2].ToString()), int.Parse(m.Groups[3].ToString()), int.Parse(m.Groups[4].ToString()));
                lights.Add(light);
            }
        ...
        public string Task1()
        {
            var lights = GetValues();

            int index = -1;
            while(!DrawCanvas(lights))
            {
                index++;
            }
        ...
        long _area = long.MaxValue;

        private bool DrawCanvas(List<Light> lights)
        {
            int x0 = lights.Min(s => s.SX);
            int xn = lights.Max(s => s.SX);
            int y0 = lights.Min(s => s.SY);
            int yn = lights.Max(s => s.SY);

            long area = (xn - x0) * (yn - y0);
            if (area > _area)
            {
                lights.ForEach(s =>
                {
                ...
                {
                    var row = lights.Where(s => s.Y == y).OrderBy(s => s.Y).ThenBy(s => s.X);
